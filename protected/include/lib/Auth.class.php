...
class Auth
{
	...
	static public function set($groupid)
	{
		...
		session(self::$config['AUTH_SESSION_PREFIX'].'groupid',$groupid);
	}
	...
	static public function check($config=array())
	{
		...
		if(false==self::checkLogin())
		{
		    ...
		    $power=self::getGroupPower(session(self::$config['AUTH_SESSION_PREFIX'].'groupid'));
  ...
  KEY `pid` (`pid`)
	...
	static public function getGroupPower($group_id='')
	{
		...
		if(self::$config['AUTH_POWER_CACHE']&&session(self::$config['AUTH_SESSION_PREFIX'].'power'))
		return session(self::$config['AUTH_SESSION_PREFIX'].'power');
